...
static int mctp_sockets_show(struct seq_file *m, void *v)
{
	...
	/* 6.12->6.18 drift: net->mctp.binds is now a hashtable (DECLARE_HASHTABLE),
	 * not a single hlist. Iterate every bucket to visit all bound sockets.
	 */
	rcu_read_lock();
	for (bkt = 0; bkt < HASH_SIZE(net->mctp.binds); bkt++)
	sk_for_each_rcu(sk, &net->mctp.binds[bkt]) {
		struct mctp_sock *msk = container_of(sk, struct mctp_sock, sk);
