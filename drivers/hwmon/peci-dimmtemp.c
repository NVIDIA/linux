...
static inline int read_ddr_dimm_temp_config(struct peci_dimmtemp *priv,
					    int chan_rank, u8 *cfg_data)
{
	return peci_client_read_package_config(
		priv->mgr, PECI_MBX_INDEX_DDR_DIMM_TEMP, chan_rank, cfg_data);
}
...
static int get_dimm_temp(struct peci_dimmtemp *priv, int dimm_no)
{
	...
	case INTEL_FAM6_SKYLAKE_X:
		 ...
		 */
		rp_msg.device =
			10 + chan_rank / 3 * 2 + (chan_rank % 3 == 2 ? 1 : 0);
	...
	for (chan_rank = 0; chan_rank < chan_rank_max; chan_rank++) {
		...
		for (dimm_idx = 0; dimm_idx < dimm_idx_max; dimm_idx++)
			if (cfg_data[dimm_idx])
				priv->dimm_mask |= BIT(
					chan_rank * dimm_idx_max + dimm_idx);
	}
...
static int create_dimm_temp_info(struct peci_dimmtemp *priv)
{
	...
	if (ret) {
		if (ret == -EAGAIN) {
			if (priv->retry_count < DIMM_MASK_CHECK_RETRY_MAX) {
				queue_delayed_work(
					priv->work_queue, &priv->work_handler,
	...
	priv->dimmtemp_label =
		devm_kzalloc(priv->dev, channels * sizeof(char *), GFP_KERNEL);
...
static void create_dimm_temp_info_delayed(struct work_struct *work)
{
	...
	struct peci_dimmtemp *priv =
		container_of(dwork, struct peci_dimmtemp, work_handler);
	int ret;
