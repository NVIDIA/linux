...
static const char *cputemp_label[DEFAULT_CHANNEL_NUMS] = {
	"Die", "DTS", "Tcontrol", "Tthrottle", "Tjmax"
};
...
static int get_temp_targets(struct peci_cputemp *priv)
{
	...
	ret = peci_client_read_package_config(
		priv->mgr, PECI_MBX_INDEX_TEMP_TARGET, 0, pkg_cfg);
	if (ret)
		return ret;
...
static int get_die_temp(struct peci_cputemp *priv)
{
	...
	priv->temp.die.value =
		priv->temp.tjmax.value + (msg.temp_raw * 1000 / 64);
...
static int get_dts(struct peci_cputemp *priv)
{
	...
	ret = peci_client_read_package_config(
		priv->mgr, PECI_MBX_INDEX_DTS_MARGIN, 0, pkg_cfg);
...
static int get_core_temp(struct peci_cputemp *priv, int core_index)
{
	...
	priv->temp.core[core_index].value =
		priv->temp.tjmax.value + core_dts_margin;
			       ...
			       u32 attr, int channel, const char **str)
{
	...
	*str = (channel < DEFAULT_CHANNEL_NUMS) ?
		       ...
		       (const char *)priv
			       ->coretemp_label[channel - DEFAULT_CHANNEL_NUMS];
...
{
	priv->coretemp_label[idx] =
		devm_kzalloc(priv->dev, PECI_HWMON_LABEL_STR_LEN, GFP_KERNEL);
...
static int create_core_temp_info(struct peci_cputemp *priv)
{
	...
	priv->coretemp_label =
		devm_kzalloc(priv->dev,
			     priv->gen_info->core_max * sizeof(char *),
			     GFP_KERNEL);
...
static int peci_cputemp_probe(struct platform_device *pdev)
{
	...
	hwmon_dev = devm_hwmon_device_register_with_info(
		priv->dev, priv->name, priv, &priv->chip, NULL);
