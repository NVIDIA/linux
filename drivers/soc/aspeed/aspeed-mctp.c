...
#define TX_PACKET_SIZE_2500(x) FIELD_PREP(GENMASK(11, 2), DATA_ADDR_2500(x))
...
/* Waiting time for Rx after Tx */
#define MCTP_CTRL_SETUP_TIME_MAX_US 1000
#define MCTP_CTRL_SETUP_TIME_MIN_US 100
...
static void aspeed_mctp_tx_cmd_prep(u32 *tx_hdr,
				    ...
				    struct mctp_pcie_packet *packet)
{
	...
	if (priv->match_data->vdm_hdr_direct_xfer) {
		...
		tx_cmd->tx_hi |=
			TX_DATA_ADDR_2500(tx->data.dma_handle + offset);
			 ...
			 struct mctp_pcie_packet *packet)
{
	...
	if (som_eom != MCTP_HDR_SOM_EOM) {
		// Aspeed driver return NULL but we need the middle packets for the application.
		//return NULL;
	...
	list_for_each_entry(handler, &priv->mctp_type_handlers, link) {
        /* Currently only one client would get registered */
...
static void aspeed_mctp_rx_tasklet(unsigned long data)
{
	...
	if (priv->match_data->vdm_hdr_direct_xfer &&
		...
		if (priv->rx_runaway_wa.packet_counter >
			    priv->rx_packet_count &&
		...
		regmap_write(priv->map, ASPEED_MCTP_RX_BUF_WR_PTR,
			     (hw_read_ptr));
...
static void aspeed_mctp_rx_chan_init(struct mctp_channel *rx)
{
	...
	struct mctp_pcie_packet_data *rx_buf =
		(struct mctp_pcie_packet_data *)rx->data.vaddr;
	int i;

	if (priv->match_data->vdm_hdr_direct_xfer) {
		for (i = 0; i < priv->rx_packet_count; i++) {
			*rx_cmd = RX_DATA_ADDR(rx->data.dma_handle +
					       data_size * i);
	...
	if (priv->match_data->fifo_auto_surround) {
		...
		priv->rx_runaway_wa.enable =
			(chip_version(priv->dev) == ASPEED_MCTP_2600) ? true :
									false;
...
static void aspeed_mctp_tx_chan_init(struct mctp_channel *tx)
{
	...
	if (priv->match_data->fifo_auto_surround) {
		regmap_write(priv->map, ASPEED_MCTP_TX_BUF_SIZE,
			     TX_PACKET_COUNT);
...
static int aspeed_mctp_open(struct inode *inode, struct file *file)
{
	...
	handler.mctp_type = MCTP_HDR_TYPE_VDM_PCI;
	handler.pci_vendor_id = 0x1ab4;
	handler.vendor_type = 0x0000;
	handler.vendor_type_mask = 0xff00;

    ret = aspeed_mctp_add_type_handler(client, handler.mctp_type,
...
int aspeed_mctp_get_eid(struct mctp_client *client, u16 bdf, u8 domain_id,
			u8 *eid)
{
	struct aspeed_mctp_endpoint *endpoint;
	int ret = -ENOENT;
...
static int aspeed_mctp_get_eid_info(struct aspeed_mctp *priv,
				    void __user *userbuf,
...
static int eid_info_cmp(void *priv, const struct list_head *a,
...
static int aspeed_mctp_set_eid_info(struct aspeed_mctp *priv,
				    void __user *userbuf,
				    enum mctp_address_type addr_format)
{
	...
	for (i = 0; i < set_eid.count; i++) {
		...
		if (addr_format == ASPEED_MCTP_EXTENDED_ADDR_FORMAT)
			...
			ret = copy_from_user(
				&endpoint->data,
				&(((struct aspeed_mctp_eid_info *)user_ptr)[i]),
				sizeof(struct aspeed_mctp_eid_info));
...
static int aspeed_mctp_set_own_eid(struct aspeed_mctp *priv,
				   void __user *userbuf)
{
...
aspeed_mctp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ASPEED_MCTP_IOCTL_GET_EID_INFO:
		ret = aspeed_mctp_get_eid_info(priv, userbuf,
					       ASPEED_MCTP_GENERIC_ADDR_FORMAT);
		break;

	case ASPEED_MCTP_IOCTL_GET_EID_EXT_INFO:
		ret = aspeed_mctp_get_eid_info(
			priv, userbuf, ASPEED_MCTP_EXTENDED_ADDR_FORMAT);
		break;
...
static void aspeed_mctp_poll_rx_queue(struct aspeed_mctp *priv)
{
	u32 status = 0;
	...
	if (priv->client_started == false) {
		schedule_delayed_work(&priv->pcie.test_dwork,
				      msecs_to_jiffies(1000));
		return;
	}
...
static void aspeed_mctp_rx_work(struct work_struct *work)
{
	struct aspeed_mctp *priv =
		container_of(work, typeof(*priv), pcie.test_dwork.work);
	struct kobject *kobj = &priv->mctp_miscdev.this_device->kobj;

	aspeed_mctp_poll_rx_queue(priv);
}
...
static void aspeed_mctp_drv_init(struct aspeed_mctp *priv)
{
	...
	// the delayed work is created to poll MCTP rx queue and MCTP RC doesn't
    // have interrupt capabilities.
	INIT_DELAYED_WORK(&priv->pcie.test_dwork, aspeed_mctp_rx_work);
...
static int aspeed_mctp_resources_init(struct aspeed_mctp *priv)
{
	...
	if (priv->rc_f) {
		priv->reset_dma =
			devm_reset_control_get_shared_by_index(priv->dev, 1);
...
static int aspeed_mctp_probe(struct platform_device *pdev)
{
	...
	// one MCTP RC is used for Nvidia platforms so the device name is aspeed-mctp.
	priv->mctp_miscdev.name = "aspeed-mctp" ;
