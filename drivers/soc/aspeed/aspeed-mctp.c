...
struct aspeed_mctp {
	...
	struct reset_control *reset_dev;
			    ...
			    struct mctp_pcie_packet *packet)
{
    ...
    /* Comment this till FPGA fixes the issue */
...
aspeed_mctp_set_eid_info(struct aspeed_mctp *priv, void __user *userbuf)
{
	...
	list_sort_old(NULL, &list, &eid_info_cmp);
...
{
	struct aspeed_mctp *priv = container_of(work, typeof(*priv),
						pcie.test_dwork.work);
	struct kobject *kobj = &aspeed_mctp_miscdev.this_device->kobj;

	aspeed_mctp_poll_irq(priv);
}
...
{
    dev_dbg(priv->dev, "Resetting MCTP-Dev controller\n");
	if (reset_control_assert(priv->reset_dev) != 0)
		dev_warn(priv->dev, "Failed to assert reset_dev\n");

	if (reset_control_deassert(priv->reset_dev) != 0)
		dev_warn(priv->dev, "Failed to deassert reset_dev\n");
}
...
static int aspeed_mctp_resources_init(struct aspeed_mctp *priv)
{
	...
	priv->reset_dev = devm_reset_control_get_exclusive(priv->dev, "mctp-dev");
	if (IS_ERR(priv->reset_dev)) {
		dev_err(priv->dev, "Failed to get mctp-dev reset!\n");
		return PTR_ERR(priv->reset_dev);
	}

    /*
     * Bug on AST2600 SoC, it is necessary to reset the MCTP-Dev contorller
     * before resetting the MCTP-RC
     */
    aspeed_mctp_dev_hw_reset(priv);

	priv->reset = devm_reset_control_get_exclusive(priv->dev, "mctp-rc");
	if (IS_ERR(priv->reset)) {
		dev_err(priv->dev, "Failed to get mctp-rc reset!\n");
