...
static irqreturn_t npcm_peci_irq_handler(int irq, void *arg)
{
	...
	regmap_write_bits(priv->regmap, NPCM_PECI_CTL_STS, NPCM_PECI_INT_MASK,
			  status_ack);
...
static int npcm_peci_init_ctrl(struct npcm_peci *priv)
{
	...
	ret = of_property_read_u32(priv->dev->of_node, "pull-down", &pull_down);
	...
	priv->host_bit_rate =
		clk_get_rate(priv->clk) / (4 * (host_neg_bit_rate + 1));
...
static int npcm_peci_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, priv->irq, npcm_peci_irq_handler, 0,
			       "peci-npcm-irq", priv);
	if (ret)
		goto err_put_adapter_dev;
	...
	dev_info(&pdev->dev,
		 "peci bus %d registered, host negotiation bit rate %dHz",
