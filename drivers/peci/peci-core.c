			 ...
			 char *buf)
{
	return sprintf(buf, "%s\n",
		       dev->type == &peci_client_type ?
			       to_peci_client(dev)->name :
			       to_peci_adapter(dev)->name);
}
...
static struct attribute *peci_device_attrs[] = { &dev_attr_name.attr, NULL };
...
{
	return (dev->type == &peci_client_type) ? to_peci_client(dev) : NULL;
}
...
static int peci_xfer_with_retries(struct peci_adapter *adapter,
				  struct peci_xfer_msg *msg, bool has_aw_fcs)
{
...
static int peci_cmd_rd_end_pt_cfg(struct peci_adapter *adapter, void *vmsg)
{
	...
	case PECI_ENDPTCFG_TYPE_MMIO:
		...
		msg = peci_get_xfer_msg(tx_size, PECI_RDENDPTCFG_READ_LEN_BASE +
							 umsg->rx_len);
		if (!msg)
			return -ENOMEM;
		...
		msg->tx_buf[10] =
			(u8)umsg->params.mmio.offset; /* LSB - Register Offset */
...
static struct attribute *peci_adapter_attrs[] = { &dev_attr_name.attr,
...
{
	return (dev->type == &peci_adapter_type) ? to_peci_adapter(dev) : NULL;
}
...
static int peci_add_numbered_adapter(struct peci_adapter *adapter)
{
	...
	mutex_lock(&core_lock);
	id = idr_alloc(&peci_adapter_idr, adapter, adapter->nr, adapter->nr + 1,
		       GFP_KERNEL);
	mutex_unlock(&core_lock);
