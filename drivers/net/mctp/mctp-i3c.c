...
static int mctp_i3c_read(struct mctp_i3c_device *mi)
{
	...
	if (xfer.len < MCTP_I3C_MINLEN) {
		...
		netdev_dbg(ndev, "MCTP I3C: RX length error %d\n", xfer.len);
	...
	if (pec != ((u8 *)xfer.data.in)[xfer.len - 1]) {
		...
		netdev_dbg(ndev, "MCTP I3C: PEC error\n");
	...
	if (net_status == NET_RX_SUCCESS) {
		...
		netdev_dbg(ndev, "MCTP I3C: RX success, %u bytes\n", xfer.len - 1);
		trace_mctp_transport_rx("i3c", ndev, 0, xfer.len - 1);
	} else {
		stats->rx_dropped++;
		netdev_dbg(ndev, "MCTP I3C: RX dropped, status=%d\n", net_status);
...
static void mctp_i3c_xmit(struct mctp_i3c_bus *mbus, struct sk_buff *skb)
{
	...
	if (rc == 0) {
		...
		stats->tx_packets++;
		netdev_dbg(ndev, "MCTP I3C: TX success, %u bytes\n", data_len);
		...
		netdev_dbg(ndev, "MCTP I3C: TX failed, error=%d\n", rc);
