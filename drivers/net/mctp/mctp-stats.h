 ...
 *
 * Value: 254 (chosen to avoid conflict with valid MCTP EIDs 0-253, and distinct
 *        from broadcast address 255)
 *
 ...
 * - UNKNOWN (254): We don't know the EID (error occurred too early)
...
\
static int prefix##_count_eid_nonzero(dev_type *dev, u8 eid) \
{ \
...
{ \
	...
	int eid; \
	\
	for_each_set_bit(eid, dev->eid_stats.active, 256) { \
...
\
static u64 prefix##_eid_stats_total(dev_type *dev, u8 eid) \
{ \
	eid_stats_type *es = &dev->eid_stats.eid[eid]; \
	u8 *base = (u8 *)es; \
	u64 total = 0; \
 ...
 * active EIDs with non-zero values. EID 254 (MCTP_EID_UNKNOWN) is displayed
 * as "UNKNOWN" instead of "EID_254".
 */
