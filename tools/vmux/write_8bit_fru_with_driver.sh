...
usage() {
	...
	echo "  PAGE_SIZE : optional, default 16 (bytes); typical 24c02 pages are 8-16"
}
...
echo "Writing 8-bit addressed EEPROM @ bus=$BUS addr=$ADDR size=${SIZE}B page=${PAGE_SIZE}B"
...
while (( OFFSET < SIZE )); do
	...
	# One-byte internal address (low 8 bits of OFFSET)
	ADDR8=$(printf "0x%02x" $(( OFFSET & 0xff )))
